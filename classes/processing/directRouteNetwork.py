...
class DirectRouteNetwork:
    ...
    def createNetwork(self) -> List[DirectRouteEntry]:
        ...
        routesAll = meshCalc.extractDirectRoutes(self._getLocBasedFeatures(
            [LevelOfCentrality.II, LevelOfCentrality.III, LevelOfCentrality.IV, LevelOfCentrality.Singular]))
        routesIII_II_S = meshCalc.extractDirectRoutes(self._getLocBasedFeatures(
            [LevelOfCentrality.II, LevelOfCentrality.III, LevelOfCentrality.Singular, LevelOfCentrality.Surrounding]))
        routesII = meshCalc.extractDirectRoutes(
            self._getLocBasedFeatures([LevelOfCentrality.II]))

        mergedRoutes = self._mergeCFRoutes(routesAll, routesIII_II_S)
        mergedRoutes = self._mergeCFRoutes(mergedRoutes, routesII)
        filteredRoutes = self._filterOutSurroundingToSurroundingRoutes(
            mergedRoutes)

        notConnectedSurroundings = self._connectNotConnectedSurroundings(
            filteredRoutes)

        return list(filteredRoutes.values()) + notConnectedSurroundings

    def _getLocBasedFeatures(
            self, locs: List[LevelOfCentrality]) -> List[CenterLayerFeature]:
        features = []
        for loc in locs:
    ...
    def _mergeCFRoutes(self, routes1: Dict[int, DirectRouteEntry],
                       routes2: Dict[int, DirectRouteEntry]) -> Dict[int, DirectRouteEntry]:
    ...
    def _filterOutSurroundingToSurroundingRoutes(
            self, routes: Dict[int, DirectRouteEntry]) -> Dict[int, DirectRouteEntry]:
        filtered = {}
    ...
    def _connectNotConnectedSurroundings(
            self, routes: Dict[int, DirectRouteEntry]) -> List[DirectRouteEntry]:
        surroundingFeats = copy.copy(
            self.centerLayer.locFeatures[LevelOfCentrality.Surrounding])

        # Remove all surrounding features from the list which already are
        # connected
        ...
        connectionCandidates = self._getLocBasedFeatures(
            [LevelOfCentrality.II, LevelOfCentrality.III, LevelOfCentrality.Singular])
        ...
        for surroundingFeat in surroundingFeats:
            ...
            nearestFeatIds = index.nearestNeighbor(QgsPointXY(
                surroundingPoint.x(), surroundingPoint.y()), 1)
            if not nearestFeatIds:
                continue
            nearestFeat = connectionCandidates[nearestFeatIds[0]]
            relationId = createDoublePointHash(
                surroundingPoint, nearestFeat.geom)
            connectedRouteEntries.append(DirectRouteEntry(
                relationId, surroundingFeat, nearestFeat))

...
class GraphkModifier:
    ...
    def modifyEdgeCostsBasedOnChangelog(
            self, minBound: float = 0.55, multiplier: float = 0.02):
        ...
        for (count, edgeIds) in countSortedChangelog.items():
            modificationFactor = max(
                minBound, self.modificationFactor - (count * multiplier))
    ...
    def modifyEdgeCosts(self, edgeIds: List[int]):
        ...
        (newEdgeIds, oldEdgeIds) = self._modifyEdgeCostInner(
            edgeIds, self.modificationFactor)
        self._modifiedEdges.update(newEdgeIds)
        self._oldEdges.update(oldEdgeIds)

    def _modifyEdgeCostInner(
            self, edgeIds: List[int], factor: float) -> Tuple[List[int], List[int]]:
        newEdgeIds = set()
        oldEdgeIds = set()

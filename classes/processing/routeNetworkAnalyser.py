...
class RouteNetworkAnalyser:
    def __init__(self, graph: QgsGraph, routeEntries: List[RouteEntry],
                 config: RouteNetworkFeatureConfig = RouteNetworkFeatureConfig()):
        self.graph = graph
        ...
        for entry in self.routeEntries:
            ...
            for edgeId in entry.routeResult.edgeIds:
                ...
                if edgeKey in collectedEntries:
                    ...
                    collectedEntries[edgeKey] = CollectedEntryItem(
                        edgeId, entry.directRouteEntry.cf)

        # aggregate values (len, CF II len, CF III len, etc.)
        networkElements = []
        for (edgeKey, entryItem) in collectedEntries.items():
            networkElement = NetworkElement(
                self.graph, edgeKey, entryItem.edgeId, entryItem.cfs)
            networkElements.append(networkElement)

        return networkElements

    def aggregateElements(self, networkElements: List['NetworkElement']
                          ) -> Tuple[List['AggregatedNetworkElement'], List['BreakingElement']]:
...
class NetworkElement:
    def __init__(self, graph: QgsGraph, edgeKey: int, edgeId: int,
                 cfs: List[ConnectivityFunction]) -> 'NetworkElement':
...
class AggregatedNetworkElement:
    def __init__(self, n: int, cf: ConnectivityFunction,
                 cfMap: Dict[ConnectivityFunction, int], geom: QgsGeometry):
        self.n = n
        self.cf = cf
        self.cfMap = cfMap
        self.geom = geom
...
class NetworkAggregator:
    ...
    def _buildPointIndex(
            self) -> Dict[Tuple[float, float], List['NetworkElement']]:
    ...
    def _elementsHaveSameProperties(
            self, elem1: 'NetworkElement', elem2: 'NetworkElement') -> bool:
        """Check if two elements have the same properties (n and cf)."""
        return elem1.n == elem2.n and elem1.cf == elem2.cf

    def _areAdjacent(self, elem1: 'NetworkElement',
                     elem2: 'NetworkElement') -> bool:
    ...
    def _getAdjacentElements(
            self, element: 'NetworkElement') -> List['NetworkElement']:
    ...
    def _splitComponentIntoLinearSegments(
            self, component: List['NetworkElement']) -> List[List['NetworkElement']]:
        ...
        for element in component:
            ...
            for other in component:
                if other.edgeKey != element.edgeKey and self._areAdjacent(
                        element, other):
                    adjacentInComponent.append(other)
            # Sort for deterministic behavior
            adjacency[element] = adjacentInComponent

        # Find branch points (elements with more than 2 neighbors) and
        # endpoints
        ...
        for startElement in sorted(startPoints, key=lambda x: x.edgeKey):
            ...
            for neighbor in adjacency[startElement]:
                ...
                segment = self._traceLinearSegment(
                    startElement, neighbor, adjacency, visitedElements)
                if segment and len(segment) > 0:
                    segments.append(segment)

        # Handle any remaining unvisited elements (shouldn't happen, but safety
        # check)
        for element in component:
    ...
    def _traceLinearSegment(self, startElement: 'NetworkElement', nextElement: 'NetworkElement',
        ...
        while currentElement and currentElement.edgeKey not in visitedElements:
            ...
            if len(nextElements) == 1:
                ...
                # Either endpoint (0 neighbors) or branch point (>1 neighbors)
                # - stop tracing
                break

        return segment

    def _buildLinestringFromLinearSegment(
            self, segment: List['NetworkElement']) -> QgsGeometry:
        ...
        for element in segment:
            ...
            adjacency[element] = sorted(
                adjacentInSegment, key=lambda x: x.edgeKey)

        # Find endpoint (element with only one neighbor) or start from first
        # element
        endpoints = [elem for elem in segment if len(adjacency[elem]) <= 1]
        startElement = endpoints[0] if endpoints else min(
            segment, key=lambda x: x.edgeKey)
        ...
        while currentElement and currentElement.edgeKey not in visitedElements:
            ...
            if previousElement is None:
                ...
                if neighbors:
                    ...
                    sharedPointKey = self._getSharedPointKey(
                        currentElement, nextElement)

                    if sharedPointKey and self._pointToKey(
                            currentElement.v1Geom) == sharedPointKey:
                        pathPoints = [currentElement.v2Geom,
                                      currentElement.v1Geom]
                    else:
                        pathPoints = [currentElement.v1Geom,
                                      currentElement.v2Geom]
                else:
                    # Single element
            ...
            nextElements = [
                elem for elem in nextElements if elem.edgeKey not in visitedElements]
    ...
    def _getSharedPointKey(self, elem1: 'NetworkElement',
                           elem2: 'NetworkElement') -> Optional[Tuple[float, float]]:
    ...
    def aggregate(
            self) -> Tuple[List[AggregatedNetworkElement], List['BreakingElement']]:
        """
        ...
        for component in components:
            ...
            for segment in linearSegments:
                ...
                aggregatedElements.append(
                    AggregatedNetworkElement(n, cf, cfMap, geom))
        ...
        (aggregatedElements, breakingPoints) = self._breakAggregationElements(
            aggregatedElements, breakingPoints)

        return (aggregatedElements, breakingPoints)

    def extractBreakingPoints(
            self, aggregatedElements: List[AggregatedNetworkElement]) -> List['BreakingElement']:
        """
        Extract breaking points where linestrings touch but don't continue.
        """
        breakingElements = {}

        def addToBreaking(point: QgsPointXY, pointIndex: int,
                          aggElem: AggregatedNetworkElement, isStartEnd: bool):
            key = self._pointToKey(point)
            if key in breakingElements:
                breakingElements[key].merge(pointIndex, aggElem, isStartEnd)
            else:
                breakingElements[key] = BreakingElement(
                    point, pointIndex, 1, aggElem, isStartEnd)
        ...
        breakingPointElements = [
            value for value in breakingElements.values() if value.count > 1]

        return breakingPointElements

    def _breakAggregationElements(self, aggregatedElements: List[AggregatedNetworkElement], breakingElements: List[
                                  'BreakingElement']) -> Tuple[List[AggregatedNetworkElement], List['SealedBreakingElement']]:
        aggregationsToSplit = defaultdict(list)
        sealedBreakingElements = []

        for breakingElement in breakingElements:
            if breakingElement.count > 1:
                sealedBreakingElements.append(
                    SealedBreakingElement(breakingElement))
                for i in range(0, len(breakingElement.pointIndices)):
                    aggregationsToSplit[breakingElement.sourceElements[i]].append(
                        breakingElement.pointIndices[i])
        ...
        for (aggregation, breakIndices) in aggregationsToSplit.items():
            ...
            for breakIndex in breakIndices:
                splittedGeom = QgsGeometry.fromPolylineXY(
                    basePolyline[fromIndex:breakIndex + 1])
                splittedAggregations.add(AggregatedNetworkElement(
                    aggregation.n, aggregation.cf, aggregation.cfMap, splittedGeom))
...
class BreakingElement:
    def __init__(self, geom: QgsPointXY, pointIndex: int, count: int,
                 aggregationElement: AggregatedNetworkElement, isStartEnd: bool):
        self.count = count
    ...
    def merge(self, pointIndex: int,
        ...
        self.cf = ConnectivityFunction.getLowerCF(
            self.cf, aggregationElement.cf)
...
class SealedBreakingElement:
    def __init__(
            self, breakingElement: BreakingElement) -> 'SealedBreakingElement':

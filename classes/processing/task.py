...
class RouteNetworkWorker(QThread):
    ...
    progressChanged = pyqtSignal(
        object)  # progress (0 - 100), status text, RouteNetworkTaskProgress
    finished = pyqtSignal(bool, object)  # success, result

    def __init__(self, networkLayerData: dict, directRouteLayerEntries:
                 List[InnerDirectRouteEntry], options: RouteGenerationOptions):
        super().__init__()
    ...
    def run(self):
        ...
        try:
            ...
            memoryLayer = RouteNetworkWorker._createLayerFromData(
                self.networkLayerData)
            ...
            self.graphGenTimer.start(self._determineGraphGenTimerTiming(
                len(self.networkLayerData["features"])))
            ...
            for entry in entries:
                isValid = self.pathFinder.areRelationCenterDistancesInTolerance(
                    entry)
            ...
            resultEntries = self._findRouteEntries(
                sortedEntries, self.options.isDetourActive(), 0, relationsLen, 55)
            ...
            if self.options.isDetourActive():
                ...
                resultEntries = self._findRouteEntries(
                    sortedEntries, False, len(entries), relationsLen, 55)

            # Step 4: Do network analysis
            self.setProgress(95, "Analysiere Umlegung..")
            networkAnalyser = RouteNetworkAnalyser(
                self.pathFinder.graph, resultEntries)
            networkElements = networkAnalyser.createNetworkElements()
            (aggregatedElements, breakingPoints) = networkAnalyser.aggregateElements(
                networkElements)

            # QgsMessageLog.logMessage("Cleaning up..")
            # self.pathFinder.cleanUp()
            QgsMessageLog.logMessage("Done.")
            result = RouteNetworkTaskResult.createSuccess(
                resultEntries, networkElements, aggregatedElements, breakingPoints)
            ...
            self.finished.emit(
                False, RouteNetworkTaskResult.createError(str(e)))
        finally:
    ...
    def _findRouteEntries(
        ...
        for cf in ConnectivityFunction:
            if cf not in sortedEntries:
                continue
            subEntries = sortedEntries[cf][0:]

            QgsMessageLog.logMessage(
                f"Finding paths for {cf.asStr()} ({len(subEntries)} relations)..")
            for (i, (isValid, routeEntry)) in enumerate(subEntries):
                ...
                routeResult = self.pathFinder.findPathOfRelation(
                    routeEntry.relationId, modifyGraph)
    ...
    def stop(self):
        ...
        self.shouldStop = True
        self.finished.emit(
            False, RouteNetworkTaskResult.createError("Stopped"))
    ...
    def _createLayerFromData(layerData: dict) -> QgsVectorLayer:
        ...
        memoryLayer = QgsVectorLayer(
            layerData['uri'], layerData['name'], "memory")
...
class RouteNetworkTask:
    ...
    def __init__(self, networkLayerData: dict, directRouteLayerEntries:
                 List[InnerDirectRouteEntry], options: RouteGenerationOptions, resultCallback, progressCallback):
        self.thread = RouteNetworkWorker(
            networkLayerData, directRouteLayerEntries, options)  # Create the thread
    ...
    @staticmethod
    def createAndRunFromContextStateHandler(
            context: DialogStateContext, resultCallback=None, progressCallback=None) -> 'RouteNetworkTask':
        ...
        if context.currentState == DialogState.REPROJECT:
            ...
            centerDistanceTolerance = context.get(
                ReprojectHandler.KCenterDistanceTolerance)
            options = RouteGenerationOptions(
                detourTolerance, centerDistanceTolerance)
            reduceNetworkBounds = context.get(
                ReprojectHandler.KReduceNetworkBounds)
            filterAttributeMode = context.get(
                ReprojectHandler.KFilterAttributeMode, NetworkSource.UNKNOWN)
        elif context.currentState == DialogState.REPROJECTDEMAND:
            networkLayer = context.get(ReprojectDemandHandler.KNetworkLayer)
            demandFieldName = context.get(
                ReprojectDemandHandler.KDemandFieldName)
            detourTolerance = context.get(
                ReprojectDemandHandler.KDetourTolerance)
            centerDistanceTolerance = context.get(
                ReprojectDemandHandler.KCenterDistanceTolerance)
            networkStrategy = DemandNetworkStrategy(
                NetworkDemandProperties.fromLayer(networkLayer, demandFieldName))
            options = RouteGenerationOptions(
                detourTolerance, centerDistanceTolerance, networkStrategy=networkStrategy)
            reduceNetworkBounds = context.get(
                ReprojectDemandHandler.KReduceNetworkBounds)
            filterAttributeMode = context.get(
                ReprojectDemandHandler.KFilterAttributeMode, NetworkSource.UNKNOWN)

        if reduceNetworkBounds:
            rect = QgsRectangle()
            for f in context.get(
                    CenterPointsHandler.KCenterLayer).qgsLayer().getFeatures():
        ...
        networkLayerData = RouteNetworkTask._createMemorylayerFromVector(
        ...
        task = RouteNetworkTask(networkLayerData, directRouteLayerEntries, options,
    ...
    @staticmethod
    def _createMemorylayerFromVector(
            originalLayer: QgsVectorLayer, request: QgsFeatureRequest) -> dict:
